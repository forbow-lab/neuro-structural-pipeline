...
for Subj in $SUBJECTS ; do 
	...
	if [ ! -r "$MDIR/${S}.MyelinMap.native.pscalar.txt" ]; then
		## CODE REFERENCE:
		...
		wb_command -cifti-convert -to-text $MDIR/${S}.MyelinMap.native.pscalar.nii $MDIR/${S}.MyelinMap.native.pscalar.txt
	
		if [ ! -r "$MDIR/${S}.MyelinMap.native.pscalar.txt" ]; then
			echo "*** ERROR: failed to create output file = $MDIR/${S}.MyelinMap.native.pscalar.txt"
			continue
		fi
	fi
	D="$S"
	for d in $(cat $MDIR/${S}.MyelinMap.native.pscalar.txt); do
		D="$D,$d"
	done
	subj_rfile=$MDIR/${S}.MyelinMap.native.pscalar.row.txt
