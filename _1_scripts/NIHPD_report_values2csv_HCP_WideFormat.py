...
def process_myelin_stats(myfile, numROI):
	if not os.path.exists(myfile):
		return ','.join(['.']*(numROI-1)) #trim first column (ssid)
	with open(myfile,'rU') as f:
		r = f.read().splitlines()[1].split(',')
	return ','.join(r[1:])
...
def report_wide_format(EDIR,ssid, isFORBOW=True):
	...
	if REPORT_DTI_VALUES:
		...
		myFile=os.path.join(SubjDir,'Myelin/%s_MyelinMap_native_pscalar_1row.csv'%(ssid))
		if not os.path.exists(myFile):
			print '* WARNING: cannot locate subjfile = %s' % (myFile)		
	...
	try:
		...
		myelinVals = process_myelin_stats(myFile,71)
		if REPORT_DTI_VALUES:
		...
		sys.exit(1)
	if REPORT_DTI_VALUES:
		return ','.join([qcstr,cnrvals,aseg,lh_aparc,rh_aparc,lh_lgi,rh_lgi,myelinVals,wm20vals,wm48vals,wmparcVals,wmHemiVals,dwiQCvals])
	else:
		return ','.join([qcstr,cnrvals,aseg,lh_aparc,rh_aparc,lh_lgi,rh_lgi,myelinVals])
...
def generate_csvfile_header(EDIR, iSubj):
	...
	myFile=os.path.join(SubjDir,'Myelin/%s_MyelinMap_native_pscalar_1row.csv'%(iSubj))
	if not os.path.exists(myFile):
		print '*** ERROR: cannot locate myelin file = %s' % (myFile)
		return hdrStr
	r1 = open(myFile,'rU').read().splitlines()[0].split(',')
	if len(r1) != 71:
		print '* WARNING: myelinFile=[%s] has only %d columns, should have 142' % (myFile,len(r1))
	else:
		r2=['Myelin_%s'%v for v in r1[1:]]
		hdrStr+=','+','.join(r2)	#skip first column (ssid)
	...
	if REPORT_DTI_VALUES:
		...
		if not os.path.exists(wmparcfile):
			print '*** ERROR: cannot locate wmparc file = %s' % (wmparcfile)
			return hdrStr
		...
		if not os.path.exists(wmHemiFile):
			print '*** ERROR: cannot locate wmHemi file = %s' % (wmHemiFile)
			return hdrStr
